...
ShowGameLayer = cc.Layer.extend({
    ...
    init: function () {
        ...
        btn_back.addTouchEventListener(this.toucheventBack, this);
        ...
        label_id.setColor(cc.color(255, 255, 255));
        label_id.setPosition(cc.p(btn_avatar.getPositionX() * 1.22, btn_avatar.getPositionY()));
        this.addChild(label_id);

        //ten hien thi
        var userName = 'Cuong_Atula';
        var label_name = MLabelCreate(userName, bk_avatar.getContentSize().height / 4, true);
        label_name.setColor(cc.color(255, 255, 255));
        label_name.setPosition(cc.p(btn_avatar.getPositionX() * 1.22,
        ...
        _bgr_xu.setTag(TAG.SHOW_BTN_NAPXU);
        // _bgr_ken.addTouchEventListener();
        var scale_xuken = (widthXuKen / 2) / (_bgr_xu.getContentSize().width);
        _bgr_xu.setScaleX(scale_xuken);
        _bgr_xu.setPosition(cc.p(btn_hopthu.getPositionX() - _bgr_xu.getContentSize().width * scale_xuken - padding,
            btn_hopthu.getPositionY() + btn_hopthu.getContentSize().height / 2 - _bgr_xu.getContentSize().height / 2));
        this.addChild(_bgr_xu);

        var sprite_xu = new cc.Sprite(res.TABLE_SPRITE_KEN);
        sprite_xu.setPosition(cc.p(_bgr_xu.getPositionX() + _bgr_xu.getContentSize().width * scale_xuken - sprite_xu.getContentSize().width + padding,
            _bgr_xu.getPositionY() + _bgr_xu.getContentSize().height / 2 - sprite_xu.getContentSize().height / 2 + padding * 1.5));
        this.addChild(sprite_xu);

        var sprite_napxu = new cc.Sprite(res.TABLE_SPRITE_NAPKEN);
        sprite_napxu.setPosition(cc.p(_bgr_xu.getPositionX() + padding * 2.2,
            _bgr_xu.getPositionY() + _bgr_xu.getContentSize().height / 2 - sprite_napxu.getContentSize().height / 2 + padding * 1.7));
        this.addChild(sprite_napxu);

        //xu
        var _bgr_ken = MCreate(res.SPRITE_BK_XUKEN);
        _bgr_ken.setTag(TAG.SHOW_BTN_NAPKEN);
        // _bgr_xu.addTouchEventListener();
        _bgr_ken.setScaleX(scale_xuken);
        _bgr_ken.setPosition(cc.p(_bgr_xu.getPositionX() - padding - _bgr_ken.getContentSize().width * scale_xuken,
            btn_hopthu.getPositionY() + btn_hopthu.getContentSize().height / 2 - _bgr_xu.getContentSize().height / 2));
        this.addChild(_bgr_ken);

        var sprite_ken = new cc.Sprite(res.SPRITE_XU);
        sprite_ken.setPosition(cc.p(_bgr_ken.getPositionX() + _bgr_ken.getContentSize().width * scale_xuken - sprite_ken.getContentSize().width + padding,
            _bgr_ken.getPositionY() + _bgr_ken.getContentSize().height / 2 - sprite_ken.getContentSize().height / 2 + padding * 1.5));
        this.addChild(sprite_ken);

        var sprite_napken = new cc.Sprite(res.TABLE_SPRITE_NAPXU);
        sprite_napken.setPosition(cc.p(_bgr_ken.getPositionX() + padding * 2.2,
            _bgr_ken.getPositionY() + _bgr_ken.getContentSize().height / 2 - sprite_napken.getContentSize().height / 2 + padding * 1.7));
        this.addChild(sprite_napken);

        var sprite_thongtin = new cc.Sprite(res.SPRITE_THONGTIN);
        sprite_thongtin.setPosition(cc.p(winsize.width / 2,
            btn_avatar.getPositionY() - sprite_thongtin.getContentSize().height * 1.6 - padding));
        ...
        bkg_navigationbar.setPosition(MVec2(bkg_navigationbar.getContentSize().width / 2, bkg_navigationbar.getContentSize().height / 2));
        this.addChild(bkg_navigationbar); //left

        // doi thuong
        var icon_doithuong_text = res.btn_doi_thuong;
        ...
        var enableGameIds = [15, 17, 19, 18, 12, 1, 11, 4, 5, 6, 2];
