...
var PopupHotLine = Popup.extend({
    ...
    init:function(){
        ...
        if (hotlines.length > 0) {
            if(hotlines.length == 1){
                var btn_call1 = MButton.createWithText(res.popup_btn_call, hotlines.length > 0 ? hotlines[0] : "",POPUP.POPUP_HOT_LINE.BTN_HOT_LINE_1);
                btn_call1.setPosition(cc.p(this.backgroundPopup.getContentSize().width - width_content / 2 - btn_call1.getContentSize().width/2,
            ...
            if(hotlines.length == 2){
                ...
                btn_call1.setPosition(cc.p(this.backgroundPopup.getContentSize().width - width_content / 2 - 5 - btn_call1.getContentSize().width,
                    this.backgroundPopup.getContentSize().height / 2 - btn_call1.getContentSize().height / 2));

                btn_call1.addTouchEventListener(this.touchEvent, this);
                this.backgroundPopup.addChild(btn_call1);

                var btn_call2 = MButton.createWithText(res.popup_btn_call, hotlines.length > 1 ? hotlines[1] : "", POPUP.POPUP_HOT_LINE.BTN_HOT_LINE_1);
        ...
        var lb_notify = MLabel.createTitle("Hỗ trợ chăm sóc khách hàng qua điện thoại viên", bg_title_popup.getContentSize().height / 2);
        lb_notify.setAnchorPoint(cc.p(0.5,0.5));
        lb_notify.setPosition(cc.p(this.backgroundPopup.getContentSize().width - width_content / 2, 3.0 * this.backgroundPopup.getContentSize().height / 4));
        this.backgroundPopup.addChild(lb_notify);

        //btn close
        var exit = MButton.create(res.IMAGE_CLOSE, POPUP.POPUP_HOT_LINE.BTN_CLOSE);
    ...
    touchEvent: function(sender,type){
        if(type == ccui.Widget.TOUCH_ENDED){
            switch (type){
                case POPUP.POPUP_HOT_LINE.BTN_HOT_LINE_1 :
                {

                }
                case POPUP.POPUP_HOT_LINE.BTN_HOT_LINE_2 :
                {

                }
                case POPUP.POPUP_HOT_LINE.BTN_FAN_PAGE :
                {

                }
                case POPUP.POPUP_HOT_LINE.BTN_CLOSE :
                {
                    this.disappear();
                }
            }
